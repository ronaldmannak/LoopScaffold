...
#
# Policy:
#   HARD BLOCK: Codex policy files and CI workflow definitions.
...
if re.search(r'(^|/)\.codex/|(^|/)\.agents/|(^|/)AGENTS\.md$|(^|/)\.github/workflows/', path):
    sys.stderr.write(f"BLOCKED: '{path}' is a policy/CI file (.codex, .agents, AGENTS.md, workflows). Changes here require a human.\n")
    sys.exit(2)
...
for e in edits:
    ...
    if SKIP.search(new) and not SKIP.search(old):
        sys.stderr.write("BLOCKED: this edit introduces a skip/disable marker into a test file.\n"
                         "Per the AGENTS.md testing rules: fix the implementation; if the test is wrong, comment for a human.\n")
        sys.exit(2)
