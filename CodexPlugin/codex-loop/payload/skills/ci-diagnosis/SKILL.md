---
name: ci-diagnosis
description: Diagnoses failed CI runs and produces a compact structured verdict so the implementing session never reads raw logs. Use this skill whenever a CI check fails — pass it the PR number and failing run/check ID. Especially valuable for long Xcode/SwiftPM logs (compiler diagnostics, macro expansion failures, linker output, parallel test noise).
---
