...
- If the repository uses more than one CI provider, require exact context names in `EXPECTED_CI_CHECKS` inside checks.sh (use the names shown by `gh pr checks`). The loop uses `--list-ci-checks` to wait for every provider to register.
...
- For slow CI only, offer `.claude/templates/claude-converge-trigger.yml` as the optional split-loop bridge. Copy it to `.github/workflows/` only with confirmation, then require the user to configure the CI workflow name, Routine B API secrets, and `CLAUDE_RUNNER_LOGIN`. Replace Routine A's watch-mode prompt with the full branch-only prompt from `"$PAYLOAD"/templates/claude-build-routine-prompt.md`, and configure Routine B with the full prompt from `"$PAYLOAD"/CONVERGE_ROUTINE_PROMPT.md`; do not summarize or reconstruct either prompt. The default single-routine loop uses only ROUTINE_PROMPT.md and does not need this workflow.
...
Print the routine name (`Implement claude-build issues`), trigger (**Issue: Labeled**, filter **Labels is one of `claude-build`**), and repo. If the user did not accept the split converger, print the FULL watch-mode prompt from `"$PAYLOAD"/ROUTINE_PROMPT.md`. If the user accepted the split converger, print the FULL branch-only Routine A prompt from `"$PAYLOAD"/templates/claude-build-routine-prompt.md` instead, then print the Routine B name (`Converge claude PRs`), API-trigger configuration, and FULL prompt from `"$PAYLOAD"/CONVERGE_ROUTINE_PROMPT.md` in its own copyable block. Each prompt opens with a /goal line: if the platform supports goal evaluation it enforces the handoff/terminal state mechanically, and otherwise reads as a plain goal statement; tell the user to check the first transcript for goal registration. Then the environment instructions: custom allowed domains (`download.swift.org`, `archive.ubuntu.com`, `security.ubuntu.com` for Swift repos) and the setup script from `.claude/templates/cloud-setup-swift.sh` in a copyable block. Then print the OPTIONAL overnight batch section: labeling multiple issues runs them in parallel (one session/branch/PR each); use "Depends-on: #N" in an issue body to serialize dependent work; and for unattended nights, offer the queue-sweeper as a second routine — SCHEDULED trigger (e.g. hourly), prompt from `"$PAYLOAD"/SWEEP_ROUTINE_PROMPT.md` in a copyable block. (In the single-routine setup the implementer posts the next-up suggestion; in split mode Routine B posts it only after convergence. The sweeper handles dependency auto-resume within the hour.) Close with: "Routine config lives in your Anthropic account — re-paste prompts whenever /loop-init reports they changed. Smoke-test with one trivial labeled issue and read the whole transcript."

## Update mode notes
When `.claude/rules` already existed before this run, this is an update: after copying, `git diff --stat .claude` and summarize what changed; explicitly tell the user whether ROUTINE_PROMPT.md, templates/claude-build-routine-prompt.md or CONVERGE_ROUTINE_PROMPT.md (when Routine B is configured), or cloud-setup-swift.sh changed, because prompt or setup changes require manual re-pasting.
