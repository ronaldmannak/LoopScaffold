...
- For slow CI only, offer `.claude/templates/claude-converge-trigger.yml` as the optional split-loop bridge. Copy it to `.github/workflows/` only with confirmation, then require the user to configure the CI workflow name, Routine B API secrets, and `CLAUDE_RUNNER_LOGIN`. The default single-routine loop does not need this workflow.
